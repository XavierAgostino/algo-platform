...
const DistanceTable = ({ 
  ...
  const getDistanceColor = (distance, nodeId) => {
    ...
    if (distance === Infinity) return 'bg-zinc-50 dark:bg-zinc-800';
    if (distance === 0) return 'bg-emerald-100 dark:bg-emerald-900/30';
    if (visitedNodes?.has(Number(nodeId))) {
      const recentlyVisited = Array.from(visitedNodes).pop() === Number(nodeId);
      if (recentlyVisited) return 'bg-indigo-100 dark:bg-indigo-900/30 animate-pulse';
  ...
  return (
    <div className="bg-white/90 dark:bg-zinc-900/90 shadow-lg rounded-lg p-3 backdrop-blur-sm border border-zinc-200 dark:border-zinc-800">
      <h3 className="text-sm font-bold mb-1 text-indigo-700 dark:text-indigo-400">Distances from Source</h3>
      <div className="border border-zinc-200 dark:border-zinc-700 rounded bg-white dark:bg-zinc-800 overflow-hidden">
        {hasData ? (
            ...
            <tbody>
              {Object.keys(distanceArray).sort().map((nodeId) => {
                ...
                return (
                  <tr 
                    key={nodeId} 
                    className={`${isVisited ? 'font-medium' : ''} ${isRecentlyUpdated ? 'bg-amber-50 dark:bg-amber-900/20' : ''}`}
                  >
                    <td className={`p-1 border-b border-zinc-200 dark:border-zinc-700 ${isVisited ? 'text-indigo-800 dark:text-indigo-300' : 'text-zinc-700 dark:text-zinc-300'}`}>
                    ...
                    <td className={`p-1 border-b border-zinc-200 dark:border-zinc-700 transition-colors duration-300 text-zinc-700 dark:text-zinc-200 ${getDistanceColor(distanceArray[nodeId], nodeId)}`}>
          ...
          <div className="text-zinc-400 dark:text-zinc-500 text-center py-3 px-2 text-sm">No data yet</div>
        )}
      </div>
    </div>
  );
